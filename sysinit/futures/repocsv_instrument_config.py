...
def copy_instrument_config_from_csv_to_mongo(data: dataBlob):
    ...
    process_modified_instruments(data_in=data_in,
                                 data_out=data_out,
                                 modified_instruments=modified_instruments)
...
def process_new_instruments(data_in,
    ...
    check_on_add = true_if_answer_is_yes("Check when adding new instruments? (say no if doing in bulk)")

    for instrument_code in new_instruments:
        instrument_object = data_in.get_instrument_data(instrument_code)
        if check_on_add:
            okay_to_add = true_if_answer_is_yes("Add %s to database?" % str(instrument_object))
            if not okay_to_add:
                continue
...
def process_modified_instruments(data_in,
    ...
    print("Existing instruments which may be updated %s" % str(actually_modified_instruments))
    check_on_modify = true_if_answer_is_yes("Check when modifying instruments? (probably should be yes except for bulk modify)")
    ...
    for instrument_code in actually_modified_instruments:
        ...
        if has_prices:
            print("%s has price data. Probably OK to change slippage but be careful about changing currency or pointsize" % instrument_code)

        if check_on_modify:
            okay_to_modify = true_if_answer_is_yes("Okay to replace \n%s with \n%s?" % (str(existing_instrument_object),
                                                                                    str(instrument_object)))
...
def process_deleted_instruments(
                       data_out,
                       deleted_instruments):
    ...
    for instrument_code in deleted_instruments:
        has_prices = instrument_code in instruments_with_prices
        if has_prices:
            print("%s has price data. ARE YOU REALLY SURE ABOUT THIS?" % instrument_code)

        if check_on_delete or has_prices:
            okay_to_delete = true_if_answer_is_yes("Okay to delete %s?" % instrument_code)
